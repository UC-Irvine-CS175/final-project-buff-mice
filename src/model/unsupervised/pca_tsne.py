...
def preprocess_images(lt_datamodule: DataLoader, num_images: int) -> tuple([np.ndarray, list]):
    """
    ...
    for batch, (img, label) in enumerate(lt_datamodule()):
        ...
        for i in range(bs):
            ...
            all_labels.append(np.argmax(label[i], axis=0).item())

            # Append the flattened image the list of all flattened images
            flat_imgs.append(temp_img)
...
def perform_tsne(X_reduced_dim: np.ndarray,
    ...
    X_embedded = tsne.fit_transform(X_reduced_dim)
