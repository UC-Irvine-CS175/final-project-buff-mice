...
class BPSDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        # train_csv file.
        tr_new_dir = os.path.join(self.train_dir, self.train_csv)
        save_tiffs_local_from_s3( self.s3_client, self.bucket_name, self.s3_path, tr_new_dir, self.train_dir)

        # call the save_tiffs_local_from_s3 function to download tiffs from the
        # val_csv file.        
        val_new_dir = os.path.join(self.val_dir, self.val_csv)
        save_tiffs_local_from_s3( self.s3_client, self.bucket_name, self.s3_path, val_new_dir, self.val_dir)

        # remember second to last param is the csv file ( full path ) and last is the parent of the csv file!
        ...
        if self.test_csv is not None:
            ...
            te_new_dir = os.path.join(self.test_dir, self.test_csv)
            save_tiffs_local_from_s3( self.s3_client, self.bucket_name, self.s3_path, te_new_dir, self.test_dir)
    ...
    def setup(self, stage: str):
        ...
        if stage == "train":
            ...
            # train_dataset = BPSMouseDataset(train_csv_file=self.train_csv, train_dir=self.train_dir, 
            #                                 resize_dims=(64, 64), meta_csv_file = self.meta_csv, meta_root_dir= self.s3_path,
            #                                 s3_client= self.s3_client, bucket_name= self.bucket_name, s3_path= self.s3_path)
            train_dataset = BPSMouseDataset(meta_csv_file=self.train_csv,
        ...
        if stage == "validate":
            ...
            validate_dataset = BPSMouseDataset(meta_csv_file=self.val_csv,
            ...
            self.val_dataset = validate_dataset
            return
            
        if stage == "test":
            # Create the BPSMouseDataset object for the test data.
            if self.test_csv is None or self.test_dir is None:
                raise FileNotFoundError("Test csv does not exist, cannot create BPSMouseDataset object for test data")
            
            test_dataset = BPSMouseDataset(meta_csv_file=self.test_csv,
...
def main():
    ...
    # downloadeded! damn that is a lot 
    # bps_dm.prepare_data()
